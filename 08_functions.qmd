...
You can download this .qmd file from [here](https://github.com/proback/264_fall_2025/blob/main/08_functions.qmd).  Just hit the Download Raw File button.
...
The problem is that `group_by()` uses data-masking rather than tidy-selection.  Even outside a function, `group_by(year, month, day)` will work, but `group_by(c(year, month, day))` will not.  The above function will work with `count_missing(flights, month, dep_time)` but not `count_missing(flights, c(year, month, day), dep_time)`.

These are the two most common subtypes of tidy evaluation:
