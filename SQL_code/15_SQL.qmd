...
You can download this .qmd file from [here](https://github.com/proback/264_fall_2025/blob/main/SQL_code/15_SQL.qmd).  Just hit the Download Raw File button.
...
Connect to St. Olaf's database server, where the flight_data database lives (note: if you are off-campus, you must first establish a VPN connection):
...
Notice that the code chunk below is actually a SQL chunk which uses SQL commands!  Running this chunk should tell us that there are 11 tables in the `flight_data` database.  These tables emulate several of the data sets found in the `nycflights13` package, except over many more years and many more airports!
...
JOIN airlines AS c ON o.Reporting_Airline = c.carrier
WHERE Year = 2019 AND Month = 6 AND DayOfMonth = 26 AND origin = 'BDL'
LIMIT 0, 6;
```
...
```{sql, connection = con}
SELECT 
  o.flight_number_reporting_airline AS flight,
...
The result from a LEFT JOIN is always a superset of the result of a (inner, regular, natural) JOIN - every row in the first data set is returned.  Here we find which flights have airports without info in the airports dataset (looks like `airports` is more complete than the airports data set in `nycflights13`).
