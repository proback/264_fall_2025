...
> The filter statement removed an entire row, not just the single NA!
...
> Recycling.  Since 1:3 is shorter than 1:6, it just repeats so that 4 gets a weight of 1, 5 gets 2, and 6 gets 3 when calculating sum(x * w).  But in our formula, the denominator only sums up 1:3.
...
> It does not put the call as part of the error message.
...
> The first is a vector of values and not a single value.
...
> sort is the name of an option in count and the name of an argument to our function.  df is not an data-variable and it's not evaluated inside a function.  n is created when we run count().
...
# 1) na.omit
na.omit(test2)
se2 <- function(x, na_remove) {
  if (na_remove) {
    x <- na.omit(x)
  }
  sqrt(var(x, na.rm = na_remove) / length(x))
}
se2(test2, na_remove = TRUE)
se2(test2, na_remove = FALSE)

# 2) !is.na
se3 <- function(x, na_remove = FALSE){
  sqrt(var(x, na.rm = na_remove) / sum(!is.na(x)))
}
se3(test2, na_remove = TRUE)
se3(test2, na_remove = FALSE)
...
```{r}
flights %>%
  summarize(arr_NA = sum(is.na(arr_delay)),
            dep_NA = sum(is.na(dep_delay)),
            both_NA = both_na(arr_delay, dep_delay)
  )
```
...
f1 calculates the difference between two times that are written as a number like in the flights data (e.g. 5:15AM = 515, and 9PM = 2100). Possible name would be `diff_times`

f2 takes variables in cm and calculates the area in inches^2. Possible name: `calc_area_inches`

f3 collapses various "non-answers". Possible name `collapse_non_answer` or `collapse_na`.  To see f3 in action:

```{r}
gss_cat %>% mutate(rinc = f3(rincome)) %>% select(rincome, rinc)

# sometimes in surveys like this there may be multiple variables that have a lot of "non-answer" options, so we could use this function to repeat the process for all those variables
```
...
```{r}
summary6 <- function(data, var, na_option = FALSE) {
  data |> summarize(
...
summary6(data = mpg_tbl, var = hwy)

summary6(data = gss_cat, var = age, na_option = TRUE)

# can also use with pipes
mpg_tbl %>%
  group_by(drv) %>%
  summary6(hwy)

# No embrace needed for a variable that's not part of a tibble
x = mpg$hwy
summary_stats2 <- function(x, na_option = FALSE) {
  tibble(x) %>%
  summarise(n = n(),
            mean = mean(x, na.rm = na_option))
}
summary_stats2(x, na_option = TRUE)
```
...
party_age |> constant_var_check(mean, sd)
