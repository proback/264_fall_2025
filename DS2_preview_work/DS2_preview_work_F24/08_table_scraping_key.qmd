...
pop3 <- as_tibble(pop2[,c(1:6,8)]) |>
  ...
  mutate(Estimate2023 = parse_number(Estimate2023),
         Census = parse_number(Census),
#         Change = parse_number(Change),  # gets rid of % and +/-
         Change = ifelse(Estimate2023 < Census, 
                         -parse_number(Change), parse_number(Change)),
         Area = parse_number(Area),
         Density = parse_number(Density)) |> 
...
outliers <- pop3 |> 
  filter(Estimate2023 > 2000000 | Area > 1500)
...
# write function to scrape data from a single year for a specific team
...
# use map function to scrape data from 2001 to 2004 and combine into 
#   a single data set
teams <- rep("MIN", 4)
years <- 2001:2004
