...
> We have statistically significant evidence (p-value = .028 from randomization test for two proportions) that the success rate is higher with dolphin therapy than without.
...
```{r}
data(iris)
iris_uniq <- vector("integer", ncol(iris))
...
iris |> summarize(
  across(.cols = everything(), .fns = n_distinct, .names = "distinct_{.col}")
)
...
# easier to run line-by-line if not part of a function
...
df |> 
  ...
  select(where(\(x) any(x > 0)))
# note that where requires a function input, and if we need an input
#   to THAT function, we create our own function inside ()
```
...
summary_stats <- function(df, 
                          ...
                          stat_fcts = list(mean = mean, sd = sd)) {
  df |> 
    summarize(
      across(.cols = {{ vars }}, 
             .fns = stat_fcts),
...
summary_stats(mtcars)
summary_stats(mtcars, vars = c(mpg, hp, wt))
summary_stats(mtcars, 
              vars = c(mpg, hp, wt), 
              stat_fcts = list(mean = mean, 
                               median = median, 
