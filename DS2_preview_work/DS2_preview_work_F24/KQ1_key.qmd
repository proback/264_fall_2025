...
2. Write a function called `summary_stats()` that allows a user to input a tibble, numeric variables in that tibble, and summary statistics that they would like to see for each variable.  Using `across()`, the function's output should look like the example below.
...
temp <- tibble(means = means)
ggplot(temp, aes(x = means)) +
  geom_histogram()
